...
func getOwnerOfNFT(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusOK,gin.H{
			"message":error in getOwnerOfNFT,
		})
		log.Fatal("error in getOwnerOfNFT\n", err)
	}
	c.JSON(http.StatusOK,gin.H{
		"message":res,
	})
...
func claimNFT(c *gin.Context) {
	...
	privateKey,StatusOK := c.GetPostForm("privateKey")
	if !StatusOK{
		panic("未获取到privateKey参数 in claimNFT")
	}
	tokenID,StatusOK := c.GetPostForm("tokenID")
	if !StatusOK{
		panic("未获取到tokenID参数 in claimNFT")
	}
	auth=generateAuth(privateKey)
	...
	_, err := conn.BuyNFT(auth, bigIntTokenID) 
	if err != nil {
		log.Fatal("createNFT error in claimNFT of test.go\n", err)
	}
	c.JSON(http.StatusOK,gin.H{
		"message":"success",
	})
