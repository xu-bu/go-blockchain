...
func getOwnOfNFT(c *gin.Context) {
	privateKey,StatusOK := c.GetPostForm("privateKey")
	if !StatusOK{
		panic("未获取到privateKey参数 in mintToContract")
	}
	...
	initVar()
	accountAddressgetAccountAddress(privateKey)
	//调用ownOf的时候不是用auth
	opts := &bind.CallOpts{
		Pending: false, // 为 true 时表示从未确认的区块中获取数据
		From:    common.HexToAddress("0xYourAddress"), // 发送者地址
	}
	bigIntTokenID := new(big.Int)
	bigIntTokenID.SetString(tokenID, 10) // Base 10
	_, err := conn.OwnerOf(opts, bigIntTokenID) // conn call the balance function of deployed smart contract
	if err != nil {
		log.Fatal("createNFT error in createNFT of test.go\n", err)
	}
	c.JSON(http.StatusOK,gin.H{
