...
func mintToContract(c *gin.Context) {
	...
	price,StatusOK := c.GetPostForm("tokenID")
	if !StatusOK{
	...
	bigIntFromString := new(big.Int)
	bigIntFromString.SetString(strValue, 10) // Base 10

	// Convert an integer to *big.Int
	bigIntFromInt := new(big.Int)
	bigIntFromInt.SetInt64(int64(intValue))
	_, err := conn.ListNFT(auth, tokenID,0) // conn call the balance function of deployed smart contract
	if err != nil {
		log.Fatal("createNFT error in createNFT of test.go\n", err)
	}
	c.JSON(http.StatusOK,gin.H{
