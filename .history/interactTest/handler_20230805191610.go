...
func initVar() {
	...
	// 注意给全局变量赋值不能用:=
	API_URL, contractAddress, account = os.Getenv("goerliURL"), os.Getenv("CONTRACT_ADDRESS"), os.Getenv("privateKey"), os.Getenv("ACCOUNT1")
	if API_URL == "" || contractAddress == "" || || account == "" {
...
func createNFT(c *gin.Context) {
	initVar()
	log.Println("init Var successfully")
